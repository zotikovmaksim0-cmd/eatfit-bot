...
def calculate_calories(gender, age, weight, height, activity_factor, goal):
    if gender == "male":
...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id in calorie_data:
        step = calorie_data[user_id].get("step")

        if step == "gender":
            gender = update.message.text.strip().lower()
            calorie_data[user_id]["gender"] = "female" if gender in ["ж","f","female"] else "male"
            calorie_data[user_id]["step"] = "age"
            await update.message.reply_text("Введите возраст:")
            return

        elif step == "age":
            calorie_data[user_id]["age"] = int(update.message.text)
            calorie_data[user_id]["step"] = "height"
            await update.message.reply_text("Введите рост (см):")
            return

        elif step == "height":
            calorie_data[user_id]["height"] = int(update.message.text)
            calorie_data[user_id]["step"] = "weight"
            await update.message.reply_text("Введите вес (кг):")
            return

        elif step == "weight":
            calorie_data[user_id]["weight"] = float(update.message.text)
            calorie_data[user_id]["step"] = "goal"
            await update.message.reply_text("Цель:\nПохудение / Поддержание / Набор")
            return

        elif step == "goal":
            goal_text = update.message.text.lower()
            ...
            calorie_data[user_id]["goal"] = goal
            calorie_data[user_id]["step"] = "activity"

            await update.message.reply_text(
                "Уровень активности:\n\n"
                "Низкий — до 7000 шагов и без тренировок\n\n"
                "Средний — 7000-12000 шагов или 2-4 тренировки в неделю\n\n"
                "Высокий — более 12000 шагов или 5+ тренировок в неделю\n\n"
                "Введите: Низкий / Средний / Высокий"
            )
            return

        elif step == "activity":
            act = update.message.text.lower()
            ...
            d = calorie_data[user_id]

            calories, protein, fat, carbs = calculate_calories(
                d["gender"], d["age"], d["weight"], d["height"], factor, d["goal"]
            )
...
async def main_menu_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if txt == "🍽 Меню":
        ...
        calorie_data[update.effective_user.id] = {"step":"gender"}
        await update.message.reply_text(
            "Выберите пол:\n\nМ - мужчина\nЖ - женщина\n\nВведите: М или Ж"
        )
